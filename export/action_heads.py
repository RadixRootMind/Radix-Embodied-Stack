...
sys.path.append(str(PROJECT_ROOT / "third_party" / "xh_model_zoo"))
import os
import torch
import numpy as np
...
workdir = os.path.join(ROOT_DIR, "artifacts/xh2/onnx")
hmonnx_file = os.path.join(ROOT_DIR, "artifacts/xh2/hmonnx")
...
def load_pi05(model_path):
    policy = PI05Policy.from_pretrained(model_path, strict=True, device=DEVICE)
    policy.to(DEVICE)
...
def run_pi05_inference(args):
    ...
    )  # 假设模块路径正确，需根据实际模型结构调整
    action_in_proj.eval()
    action_in_proj = action_in_proj.float()
    temp_onnx_action_in = os.path.join(workdir, "pi05_action_in_proj.onnx")
    simplified_onnx_action_in = os.path.join(
        workdir, "pi05_action_in_proj_so101_simplified.onnx"
    )
    input_action_in = torch.randn(
        1, 50, policy.config.max_action_dim, dtype=torch.float32
    )  # 输入形状匹配max_action_dim
    ...
    sess_action_in = ort.InferenceSession(
        simplified_onnx_action_in,
        # providers=["CUDAExecutionProvider"]
    )
    onnx_out_action_in = sess_action_in.run(
        None, {"action_in": input_action_in.cpu().numpy()}
    )
    onnx_out_np_action_in = onnx_out_action_in[0]
    print(
        "action_in_proj max abs diff:",
        np.max(np.abs(torch_out_np_action_in - onnx_out_np_action_in)),
    )
    print(
        "action_in_proj mean abs diff:",
        np.mean(np.abs(torch_out_np_action_in - onnx_out_np_action_in)),
    )
    print(
        "action_in_proj allclose:",
        np.allclose(
            torch_out_np_action_in, onnx_out_np_action_in, rtol=1e-3, atol=1e-4
        ),
    )
    ...
    temp_onnx_action_out = os.path.join(workdir, "pi05_action_out_proj.onnx")
    simplified_onnx_action_out = os.path.join(
        workdir, "pi05_action_out_proj_so101_simplified.onnx"
    )
    ...
    sess_action_out = ort.InferenceSession(
        simplified_onnx_action_out,
        # providers=["CUDAExecutionProvider"]
    )
    onnx_out_action_out = sess_action_out.run(
        None, {"action_out": input_action_out.cpu().numpy()}
    )
    onnx_out_np_action_out = onnx_out_action_out[0]
    print(
        "action_out_proj max abs diff:",
        np.max(np.abs(torch_out_np_action_out - onnx_out_np_action_out)),
    )
    print(
        "action_out_proj mean abs diff:",
        np.mean(np.abs(torch_out_np_action_out - onnx_out_np_action_out)),
    )
    print(
        "action_out_proj allclose:",
        np.allclose(
            torch_out_np_action_out, onnx_out_np_action_out, rtol=1e-3, atol=1e-4
        ),
    )
    ...
    time_mlp = TimeMLPWrapper(
        policy.model.time_mlp_in, policy.model.time_mlp_out
    )  # 假设模块路径正确
    time_mlp.eval()
    time_mlp = time_mlp.float()
    temp_onnx_time_mlp = os.path.join(workdir, "pi05_time_mlp_so101.onnx")
    simplified_onnx_time_mlp = os.path.join(
        workdir, "pi05_time_mlp_so101_simplified.onnx"
    )
    input_time_mlp = torch.randn(
        1, 1024, dtype=torch.float32
    )  # 输入形状匹配action_expert_config.width
    ...
    sess_time_mlp = ort.InferenceSession(
        simplified_onnx_time_mlp,
        # providers=["CUDAExecutionProvider"]
    )
    onnx_out_time_mlp = sess_time_mlp.run(
        None, {"time_emb": input_time_mlp.cpu().numpy()}
    )
    onnx_out_np_time_mlp = onnx_out_time_mlp[0]
    print(
        "time_mlp max abs diff:",
        np.max(np.abs(torch_out_np_time_mlp - onnx_out_np_time_mlp)),
    )
    print(
        "time_mlp mean abs diff:",
        np.mean(np.abs(torch_out_np_time_mlp - onnx_out_np_time_mlp)),
    )
    print(
        "time_mlp allclose:",
        np.allclose(torch_out_np_time_mlp, onnx_out_np_time_mlp, rtol=1e-3, atol=1e-4),
    )
