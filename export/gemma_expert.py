...
def main(args):
    ...
    cfg_name = f"{cfg_name}"
    cfg.work_dir = str(PROJECT_ROOT / "artifacts" / "xh2" / "hmonnx" / cfg_name)
    ...
    policy.model.paligemma_with_expert.gemma_expert.model.config._attn_implementation = (
        "eager"
    )

    assert isinstance(
        xh_model, XHGemma05CLLMModel
    ), f"Model must be XHGemma05CLLMModel, but got {type(xh_model)}"
    ...
    padding = torch.full(
        (1, pad_len), 2, dtype=input_ids.dtype, device=input_ids.device
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="./config/pi0/llm/pi05_gemma_expert_300m_xh2a_2k_so101_mask.py",
    )
