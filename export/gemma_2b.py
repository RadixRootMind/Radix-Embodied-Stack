...
def main(args):
    ...
    assert isinstance(
        xh_model, XHGemmaLLMModel
    ), f"Model must be XHGemmaLLMModel, but got {type(xh_model)}"
    ...
    embedding_path = PROJECT_ROOT / "artifacts" / "xh2" / "embedding.pt"
    embedding_path.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy(token_embedding_file, embedding_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="./config/pi0/llm/pi05_gemma_2b_xh2a_2k_so101_mask.py",
    )
